...
# GENERAL PERTUBATIONS LISTED BELOW
...
class CelesTrakFetcher:
    ...
    MIN_REQUEST_INTERVAL_S = 2.0 # seconds between requests
    ...
    def _get(self, url: str, cache_key: Optional[str] = None) -> Optional[str]:
        """HTTP GET with rate limiting and optional file cache."""
...
class SpaceTrackFetcher:
    ...
    def authenticate(self) -> bool:
        """Log in to Space-Track. Called automatically on first request."""
        try:
...
def parse_tle_text(raw_text: str, source: str = "unknown") -> list[TLERecord]:
    ...
    while i < len(lines):
        # Determine if current line is a name or TLE line
        if lines[i].startswith("1 ") and len(lines[i]) == 69:
            # 2-line format — no name provided
            name = "UNKNOWN"
...
def _build_tle_record(name: str, line1: str, line2: str, source: str) -> Optional[TLERecord]:
    """
    Construct a TLERecord from raw TLE strings.
    Validates checksum and parses epoch.
...
def _validate_tle_checksum(line: str) -> bool:
    """
    TLE lines end with a modulo-10 checksum digit.
    Validates that the line is internally consistent.
    """
    if len(line) != 69:
        return False
    total = 0
...
def load_tle_file(path: Path, source: str = "file") -> list[TLERecord]:
    """Load TLEs from a local .tle or .txt file."""
